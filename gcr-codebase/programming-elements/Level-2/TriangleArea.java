// creating a class TriangleArea to find area of a triangle
import java.util.Scanner;

class TriangleArea {
    public static void main(String[] args) {

        // Creating Scanner object to take user input
        Scanner sc = new Scanner(System.in);
        
        // Taking user input for base and height 
        double base = sc.nextDouble();
        double height = sc.nextDouble();

        // Calculating area in square inches
        double areaInSqInches = 0.5 * base * height;

        // Converting square inches to square centimeters
        // 1 sq inch = 6.4516 sq cm
        double areaInSqCm = areaInSqInches * 6.4516;
	
        // Displaying the result
        System.out.println("The area of the triangle in sq in is " + areaInSqInches +" and in sq cm is " + areaInSqCm);
    }
}
